...
#colors the heatmap based on row position/number - Make sure the position of color match the actual position of the correct donor.

def colorbars():

for column in meta.columns[1:]:
	species = column
	locals()[f"lut{str(column)}"] = dict(zip(species.unique(), sns.color_palette("tab10"))
	

#heatmap:
PAM50 = meta.pop("PAM50")
lut = dict(zip(PAM50.unique(), sns.color_palette("tab10")))
col_colors = PAM50.map(lut)
